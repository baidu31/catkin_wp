...
public:
    ...
    std::vector<byte> writeQueue;
    boost::shared_array<byte> writeBuffer; ///< Data being written
    size_t writeBufferSize; ///< Size of writeBuffer
    boost::mutex writeQueueMutex; ///< Mutex for access to writeQueue
    byte readBuffer[AsyncSerial::readBufferSize]; ///< data being read
    byte readBuffer1[AsyncSerial::readBufferSize];
    byte readBuffer2[AsyncSerial::readBufferSize];
    /// Read complete callback
    boost::function<void (const byte*, size_t)> callback;
};
...
void AsyncSerial::write(const byte *data, size_t size)
{
    {
...
void AsyncSerial::write(const std::vector<byte>& data)
{
    {
        lock_guard<mutex> l(pimpl->writeQueueMutex);
...
void AsyncSerial::doWrite()
{
    ...
    if(pimpl->writeBuffer==0)
    {
        ...
        pimpl->writeBuffer.reset(new byte[pimpl->writeQueue.size()]);
...
void AsyncSerial::setReadCallback(const boost::function<void (const byte*, size_t)>& callback)
{
...
void AsyncSerial::setReadCallback(const
        function<void (const byte*, size_t)>& callback)
{
    pimpl->callback=callback;
}
