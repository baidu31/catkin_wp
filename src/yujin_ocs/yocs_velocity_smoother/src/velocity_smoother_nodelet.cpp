...
#define IS_ZERO_VEOCITY(a)   ((a.linear.x == 0.0) && (a.linear.y == 0.0) &&(a.angular.z == 0.0))         //a.linear.y == 0.0
...
void VelocitySmoother::velocityCB(const geometry_msgs::Twist::ConstPtr& msg)
{
  ...
  {
    ROS_INFO("period_record.size()    %d",  period_record.size());
    period_record.push_back((ros::Time::now() - last_cb_time).toSec());
    ROS_INFO("period_record    %f",  period_record.back());
  }
  else
  {
    ROS_INFO("pr_next    %d",  pr_next);
...
void VelocitySmoother::spin()
{
  ...
  while (! shutdown_req && ros::ok())
  {
    ...
    double vx_deviation_lower_bound = last_cmd_vel.linear.x - decel_lim_v * period * period_buffer;
    double vx_deviation_upper_bound = last_cmd_vel.linear.x + accel_lim_v * period * period_buffer;

    double vy_deviation_lower_bound = last_cmd_vel.linear.y - decel_lim_v * period * period_buffer;
    double vy_deviation_upper_bound = last_cmd_vel.linear.y + accel_lim_v * period * period_buffer;
    ...
    bool vy_different_from_feedback = current_vel.linear.y < vy_deviation_lower_bound || current_vel.linear.x > vy_deviation_upper_bound;
        ...
        (target_vel.angular.z != last_cmd_vel.angular.z))
    {
      ...
      {
        max_vy_inc = ((vy_inc*target_vel.linear.y > 0.0)?accel_lim_v:decel_lim_v)*period;
      }
      ...
      // double Av = (  std::abs(vx_inc)  + std::abs(vy_inc) ) / MA;
      // double Aw = std::abs(w_inc) / MA;
      // double Bv = (  max_vx_inc +  max_vy_inc  )/ MB;
      // double Bw = max_w_inc / MB;
      // double theta = atan2(Bw, Bv) - atan2(Aw, Av);
      double Av = sqrt(    vx_inc *     vx_inc +   vy_inc *     vy_inc  ) / MA;
      double Aw = std::abs(w_inc) / MA;
      double Bv = sqrt(max_vx_inc * max_vx_inc +   max_vy_inc * max_vy_inc )/ MB;
      double Bw = max_w_inc / MB;
      double theta = atan2(Bw, Bv) - atan2(Aw, Av);
      ...
      else
      {
        ...
        max_w_inc = ((max_vx_inc+max_vy_inc)*std::abs(w_inc))/ (std::abs(vx_inc)+ std::abs(vy_inc));
      }

      if (std::abs(vx_inc) > max_vx_inc)
      {
        // we must limit linear velocity x
        cmd_vel->linear.x  = last_cmd_vel.linear.x  + sign(vx_inc)*max_vx_inc;
      }

      if (std::abs(vy_inc) > max_vy_inc)
      {
        // we must limit linear velocity y
