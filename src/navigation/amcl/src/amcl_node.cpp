        ...
        first_reconfigure_call_(true)
{
  ...
  laser_scan_filter_->registerCallback(boost::bind(&AmclNode::laserReceived,
                                                   this, _1));
  initial_pose_sub_ = nh_.subscribe("initialpose", 2, &AmclNode::initialPoseReceived, this);       //调用handleInitialPoseMessage， pf_init()处理初始位置坐标

  if(use_map_topic_) {      //When set to true, AMCL will subscribe to the map topic rather than making a service call to receive its map.
    map_sub_ = nh_.subscribe("map", 1, &AmclNode::mapReceived, this);            //运行handleMapMessage，初始化AMCLOdom，AMCLLaser
...
AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr& laser_scan)
{
  ...
  if(frame_to_laser_.find(laser_scan->header.frame_id) == frame_to_laser_.end())
  {
    ...
    try
    {
      this->tf_->transformPose(base_frame_id_, ident, laser_pose);  //激光原点在车体坐标系下base_link的坐标
    }
    catch(tf::TransformException& e)
    {
  ...
  else if(pf_init_ && lasers_update_[laser_index])
  {
    ...
    odom_->UpdateAction(pf_, (AMCLSensorData*)&odata);             //根据里程计的变化值+高斯噪音 更新 pf_t中samples的内里程计值（运动模型）
  ...
  if(lasers_update_[laser_index])
  {
    ...
    tf::Stamped<tf::Quaternion> inc_q(q, laser_scan->header.stamp,
                                      laser_scan->header.frame_id);            //处于激光坐标系
    try
    {
      tf_->transformQuaternion(base_frame_id_, min_q, min_q);                  //转换到base_link
    ...
    // Apply range min/max thresholds, if the user supplied them             激光的测距量程
    ...
    ldata.ranges = new double[ldata.range_count][2];          //二维数组
    ...
    lasers_[laser_index]->UpdateSensor(pf_, (AMCLSensorData*)&ldata);        // 根据当前雷达数据更新各里程计对应的权值weights  （观测模型   p（z|x）=   1/sqrt(2*pi*σ^2)) * exp(-(z-μ  )^2/(2*σ^2))）               
    ...
    {
      pf_update_resample(pf_);                    //更新采样里程计值
      resampled = true;
    }
  ...
  if(resampled || force_publication)
  {
        ...
        hyp_count < pf_->sets[pf_->current_set].cluster_count; hyp_count++)
    {
      ...
      if (!pf_get_cluster_stats(pf_, hyp_count, &weight, &pose_mean, &pose_cov))     // 根据聚类获取权值最高的聚类的统计特性，即为当前机器人所在位置坐标
      {
    ...
    if(max_weight > 0.0)
    {
      ...
      pose_pub_.publish(p);                            //发布  /amcl_pose
      ...
      try
      {
        ...
        tf::Stamped<tf::Pose> tmp_tf_stamped (tmp_tf.inverse(),
                                              ...
                                              base_frame_id_);                 //map坐标系原点在 base_link坐标系下的表示
        this->tf_->transformPose(odom_frame_id_,
                                 tmp_tf_stamped,
                                 odom_to_map);                                 //map坐标系原点在 odom坐标系下的表示
      }
