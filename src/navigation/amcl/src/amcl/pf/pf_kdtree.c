...
////////////////////////////////////////////////////////////////////////////////
// Insert a pose into the tree.                                                                                          // Add sample to histogram
void pf_kdtree_insert(pf_kdtree_t *self, pf_vector_t pose, double value)                    //pf_kdtree_insert(set->kdtree, sample->pose, sample->weight);
{
  int key[3];
...
// should do a kernel density estimate rather than a simple histogram.    核密度估计   简单的直方图 
                                        ...
                                        pf_kdtree_node_t *node, int key[], double value)     //pf_kdtree_insert_node(self, NULL, self->root, key, value)
{
  ...
  else if (node->leaf)                           //第二次调用　　该　if 成立   self->root　已经有了，而且是叶节点
  {
    ...
    else
    {
      ...
      if (key[node->pivot_dim] < node->pivot_value)       // 等价与  key[node->pivot_dim] < node->key[node->pivot_dim]
      {
        node->children[0] = pf_kdtree_insert_node(self, node, NULL, key, value);                //self->leaf_count += 1;
        node->children[1] = pf_kdtree_insert_node(self, node, NULL, node->key, node->value);   //self->leaf_count += 1;   ？？？？岂不是和父节点的key和value一样
      }
      else
      {
...
////////////////////////////////////////////////////////////////////////////////
// Recursive node search
pf_kdtree_node_t *pf_kdtree_find_node(pf_kdtree_t *self, pf_kdtree_node_t *node, int key[])   //node = pf_kdtree_find_node(self, self->root, key);
{
  if (node->leaf)
  {
  ...
  else
  {
    ...
    if (key[node->pivot_dim] < node->pivot_value)
      return pf_kdtree_find_node(self, node->children[0], key);  // //由于总有一个子节点和父节点的kye和value一样，最后的迭代总会到叶节点上
    else
...
void pf_kdtree_cluster_node(pf_kdtree_t *self, pf_kdtree_node_t *node, int depth)            //????????
{
  ...
  {                                                                                        //???聚类   何为聚类
    nkey[0] = node->key[0] + (i / 9) - 1;                                 //恍然大悟  就是在三维方向进行-1,0,1的平移，一共27种方式
