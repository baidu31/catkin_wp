...
void LaserscanMerger::laserscan_topic_parser()
{
	...
	for(int i=0;i<tokens.size();++i)                               //   取出给定的并且有的laser topic
	{
	...
	std::vector<string>::iterator last = std::unique(tmp_input_topics.begin(), tmp_input_topics.end());     // 去除相邻重复topic，重复的移动后面去，然后返回去重后最后一个元素的地址
	tmp_input_topics.erase(last, tmp_input_topics.end());   //删除
	...
	{
                //ROS_INFO("scan_subscribers.size():   %ld",  scan_subscribers.size());
...
void LaserscanMerger::scanCallback(const sensor_msgs::LaserScan::ConstPtr& scan, std::string topic)
{
	...
	cout << "scan.intensities.size:   "  << scan->intensities.size() << endl;
	projector_.transformLaserScanToPointCloud(scan->header.frame_id, *scan, tmpCloud1, tfListener_, 0x02);
	cout << "tmpCloud1:   "   <<  tmpCloud1.channels.size()  << endl;
...
cout<< "input_topics.size() " <<  input_topics.size()  << endl;
	for(int i=0; i<input_topics.size(); ++i)
	{
		if(topic.compare(input_topics[i]) == 0)
		{
		        cout << "tmpCloud2:   "   <<  tmpCloud2.channels.size()  << endl;
			...
			cout <<"tmpCloud3.fields.size ()" << "    "  << tmpCloud3.fields.size() << endl;
		}
	}	
	...
	if(totalClouds == clouds_modified.size())
	{
		...
		cout<< "clouds_modified.size():  " <<  clouds_modified.size()  << endl;
		cout <<"size:   "<<   clouds[0].fields.size () << "    "  <<  clouds[1].fields.size() << endl;
