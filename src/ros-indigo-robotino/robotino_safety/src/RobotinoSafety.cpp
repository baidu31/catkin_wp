...
RobotinoSafety::RobotinoSafety():
	...
	node_loop_rate_(20)
{
	cmd_vel_pub_ = nh_.advertise<geometry_msgs::Twist>("/robotino_cmd_vel", 10);
	...
	robotino_cmd_vel_sub_ = nh_.subscribe("/cmd_vel", 10, &RobotinoSafety::robotinoCmdVelCallback, this);
	bumper_sub_ = nh_.subscribe("/bumper", 10, &RobotinoSafety::bumperCallback, this);
	scan_sub_ = nh_.subscribe("/scan", 10, &RobotinoSafety::scanCallback, this);
...
void RobotinoSafety::scanCallback(const sensor_msgs::LaserScanConstPtr& msg)
{
	...
	try
	{
		tfListener_.waitForTransform("/base_link", "/laser_link", ros::Time(0), ros::Duration(0.1));
